...
export function InviteAccept() {
  ...
  async function accept(event: FormEvent<HTMLFormElement>) {
    ...
    if (!token)
      return notify("warning", "This invitation link is missing its token.");
    ...
    <main className="grid min-h-screen place-items-center bg-slate-50 px-4 py-10">
      <section className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-6 shadow-xl shadow-slate-200/50 sm:p-8">
        ...
        <p className="mt-2 text-sm leading-6 text-slate-600">
          Set your name and password to activate your invited account.
        </p>
        <form className="mt-7 space-y-4" noValidate onSubmit={accept}>
        ...
        {!token && (
          <p className="mt-4 rounded-lg bg-amber-50 p-3 text-sm text-amber-800">
            This invitation link is incomplete. Ask your organization admin for
            a new link.
          </p>
        )}
      </section>
