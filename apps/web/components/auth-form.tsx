...
export function AuthForm({ mode }: { mode: Mode }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!email.trim())
      return notify("warning", "Enter your email address to continue.");
    if (!password) return notify("warning", "Enter your password to continue.");
    if (isSignup && !organizationName.trim())
      return notify("warning", "Enter your organization name to continue.");
    ...
    try {
      const response = await fetch(`/api/auth/${mode}`, {
        ...
        body: JSON.stringify(
          isSignup
            ? { email, password, organizationName, displayName }
            : { email, password },
        ),
      });
      ...
      notify(
        "success",
        isSignup ? "Your workspace has been created." : "You are signed in.",
      );
      router.replace("/");
    } catch (cause) {
      notify(
    ...
    <main className="grid min-h-screen place-items-center bg-slate-50 px-4 py-10">
      <section className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-6 shadow-xl shadow-slate-200/50 sm:p-8">
        ...
        <p className="mt-2 text-sm leading-6 text-slate-600">
          {isSignup
            ? "Start turning customer feedback into clear next steps."
            : sessionExpired
              ? "Your session expired. Sign in again to continue."
              : "Sign in to your customer intelligence workspace."}
        </p>
            ...
            <>
              <label className="block text-sm font-medium text-slate-700">
                ...
                <Input
                  ...
                  autoComplete="name"
                  className="mt-1.5 w-full rounded-lg border border-slate-300 px-3 py-2.5 outline-none transition focus:border-blue-600 focus:ring-2 focus:ring-blue-100"
                />
              </label>
              <label className="block text-sm font-medium text-slate-700">
                Organization name
