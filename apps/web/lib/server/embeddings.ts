...
export async function findSimilarFeedback(client: PoolClient, tenantId: string, queryEmbedding: number[], limit = 10, threshold = 0.62): Promise<SimilarFeedback[]> {
  ...
  const result = await client.query<SimilarFeedback>(`SELECT f.id, f.raw_text, COALESCE(j.original_filename, f.source) AS source, f.occurred_at, 1 - (e.embedding <=> $2::vector) AS similarity,
      (1 - (e.embedding <=> $2::vector)) + 0.05 * exp(-extract(epoch FROM (now() - COALESCE(f.occurred_at, f.created_at))) / 15552000) AS score
    FROM feedback_embeddings e JOIN feedback_items f ON f.id = e.feedback_item_id LEFT JOIN ingestion_jobs j ON j.id = f.ingestion_job_id
