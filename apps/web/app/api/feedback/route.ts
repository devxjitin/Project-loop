...
export async function GET(request: NextRequest) {
  try {
    ...
    const items = await withTransaction(async (client) => {
      ...
      if (source) add('f.source = ?', source); if (sentiment) add('f.sentiment = ?', sentiment); if (from) add('f.created_at >= ?::timestamptz', from); if (to) add("f.created_at < (?::timestamptz + interval '1 day')", to);
