...
export async function POST(request: NextRequest) {
  ...
  } catch (error) { if (error instanceof AuthError) return Response.json({ error: error.message }, { status: error.status }); console.error('Q&A answer failed', error); const detail = error instanceof Error ? error.message : ''; if (/\((502|503|504)\)|fetch failed|timeout|aborted/i.test(detail)) return Response.json({ error: 'The AI service is waking up. Please try again in about a minute.' }, { status: 503 }); if (/\(429\)/.test(detail)) return Response.json({ error: 'The AI service is busy right now. Please try again in a moment.' }, { status: 429 }); return Response.json({ error: 'Unable to produce a grounded answer.' }, { status: 502 }); }
}
