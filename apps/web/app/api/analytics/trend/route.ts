...
import { dateRange, trend } from '@/lib/server/analytics';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const data = await withTransaction(async (client) => { await setTenantContext(client, claims.tenantId); return trend(client, claims.tenantId, from, to); });
